...
* Journal
*** COMMENT org-journal
Based on config [[https://github.com/rememberYou/.emacs.d/blob/master/config.org#journal][here]].
Note that if you backfill entries, it will mess up navigating to next / previous entries. Use =M-x org-journal-invalidate-cache= to fix.

As of December 2020, switched to org-roam-dailies instead.
#+BEGIN_SRC emacs-lisp
(use-package org-journal
  ...
  :custom
  (org-journal-date-format "W%W: %e %b %Y (%A)\nRemember to write single sentence summary (C-c c D)")
  (org-journal-dir "~/Dropbox/org/gtd/journal/")
  (org-journal-enable-encryption nil) ; It's annoying to have to decrypt all the time
